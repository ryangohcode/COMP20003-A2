...
df = df.sort_values('% Dose 2',ascending=False).tail(10)
#produce bar chart
print(df.to_string())
lga_column = df["LGA"]
...
plt.suptitle('Bar chart of the top 10 least percentage of people vaccinated against COVID-19 by LGA in Victoria', fontsize=8, fontweight='bold')
plt.savefig('../visualisation_graphs/least-vaccinated_barchart.png')
plt.show()
plt.close()